...
ARTIFACT_PATH="${ARTIFACT_ROOT}/equndo-flatpak-source-tree.tar.gz"
...
dart tool/gen_l10n.dart

bash "${SCRIPT_DIR}/../linux/patch_plugins.sh"
...
(
  ...
  python3 "scripts/ci/flatpak/refresh_pub_advisories_cache.py" "${SOURCE_TREE_ROOT}/.pub-cache"
)

echo "==> Vendoring Cargo dependencies for offline Linux builds"
(
  cd "${SOURCE_TREE_ROOT}"
  mapfile -t RUST_MANIFESTS < <(python3 "scripts/ci/linux/list_rust_manifests.py" "${SOURCE_TREE_ROOT}")
