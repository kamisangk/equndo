...
class _TagTopicsPageState extends ConsumerState<TagTopicsPage> {
  ...
  Future<void> _loadTopics() async {
    ...
    try {
      final response = await _fetchTaggedTopicResponse(page: 1);
  ...
  Future<void> _silentRefresh() async {
    try {
      ...
      debugPrint('[TagTopicsPage] silent refresh failed: $e\n$s');
    }
  }
  ...
  Future<void> _loadMore() async {
    ...
    try {
      ...
      final response = await _fetchTaggedTopicResponse(page: nextPage + 1);
  ...
  Future<TopicListResponse> _fetchTaggedTopicResponse({
    required int page,
  }) async {
    ...
    final response = EqunDiscuzAdapter.guideTopicsToResponse(
      topics,
      hasMore: topics.isNotEmpty,
    );
    final normalizedTag = widget.tagName.toLowerCase();
