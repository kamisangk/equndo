  ...
  AsyncValue _watchSourceState(WidgetRef ref) {
    switch (_effectiveSource) {
      ...
      case StatsDataSource.equn:
        return ref.watch(equnDiscuzProfileProvider);
    }
  }
  ...
  Map<ProfileStatType, int> _resolveValues(
    ...
    switch (config.dataSource) {
      ...
      case StatsDataSource.connect:
        ...
        return _fromSummary(ref.watch(userSummaryProvider).value);
      case StatsDataSource.equn:
        return _fromEqun(ref.watch(equnDiscuzProfileProvider).value);
    }
  }
...
class _DataSourceSelector extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Row(
      children: [
        for (final source in profileStatsVisibleDataSources)
          Padding(
...
class _StatChipGrid extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final notifier = ref.read(profileStatsConfigProvider.notifier);
    final available = supportedStatsPerSource[config.dataSource] ?? const {};
    final selected = config.enabledStats
        .where((s) => available.contains(s))
        .toList();
    final unselected = available.where((s) => !selected.contains(s)).toList();
