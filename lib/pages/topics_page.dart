...
import '../utils/equn_search_launcher.dart';
...
class _TopicsPageState extends ConsumerState<TopicsPage>
            ...
            onPressed: () {
              ...
              if (id != null) {
                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (_) => EqunThreadDetailPage(tid: id),
                  ),
                );
              }
            },
  ...
  /// 构建排序栏右侧的按钮
  /// - 新/未读排序且已登录时：显示忽略按钮
  Widget? _buildTrailing(bool isLoggedIn, TopicListFilter currentFilter) {
    // 新/未读筛选时显示忽略按钮
    if (isLoggedIn &&
  ...
  Widget build(BuildContext context) {
    ...
    final currentCategoryId = _currentCategoryId(pinnedIds);
    final hasSelectedCategory = currentCategoryId != null;
            ...
            headerSliverBuilder: (context, innerBoxIsScrolled) => [
              SliverPersistentHeader(
                ...
                delegate: _TopicsHeaderDelegate(
                  ...
                  onSearch: () => EqunSearchLauncher.open(context),
                  onDebugTopicId: () => _showTopicIdDialog(context),
                  trailing: _buildTrailing(isLoggedIn, currentFilter),
                ),
              ),
            ],
            body: Column(
...
class _TopicsHeaderDelegate extends SliverPersistentHeaderDelegate {
  ...
  bool shouldRebuild(covariant _TopicsHeaderDelegate oldDelegate) {
    return statusBarHeight != oldDelegate.statusBarHeight ||
        ...
        hasSelectedCategory != oldDelegate.hasSelectedCategory ||
          ...
          SizedBox(
            ...
            child: Row(
              children: [
                ...
                // 筛选/排序栏隐藏时，渐显筛选快捷按钮
                if (!hasSelectedCategory && sortProgress > 0)
                  Opacity(
                  ...
                  child: Consumer(
                    builder: (context, ref, _) {
                      ...
                      return SortAndTagsBar(
                        ...
                        onAddTag: onAddTag,
                        trailing: trailing,
                        showFilter: !hasSelectedCategory,
                        showSort: hasSelectedCategory,
                        showTags: false,
                        sortOptions: homeTopicSortOptions,
                      );
                    },
                  ),
                ),
  ...
  List<Tab> _buildTabs(BuildContext context) {
    final tabs = <Tab>[Tab(text: context.l10n.common_all)];
...
class _TopicListState extends ConsumerState<_TopicList>
  ...
  void _openFocusedTopic(AsyncValue<List<Topic>> topicsAsync) {
    ...
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) =>
            EqunThreadDetailPage(tid: topic.id, initialTitle: topic.title),
      ),
    );
  }
  ...
  Future<void> _triggerLoadMore(int? providerKey) async {
    ...
    final keywords = widget.categoryId == null
        ? const <String>[]
        : prefs.normalizedFilterKeywords;
  ...
  Widget build(BuildContext context) {
    ...
    final useKeywordFilter = widget.categoryId != null;
    final effectiveKeywords = useKeywordFilter ? keywords : const <String>[];
    _syncAutoLoadFilter(effectiveKeywords, wholeWord);
    var hiddenCount = 0;
    final visibleTopicsAsync = topicsAsync.whenData((topics) {
      final (visible, hidden) = TopicKeywordFilter.apply(
        topics,
        normalizedKeywords: effectiveKeywords,
        wholeWord: wholeWord,
      );
      hiddenCount = hidden;
      return visible;
