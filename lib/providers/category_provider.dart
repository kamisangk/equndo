...
final categoriesProvider = FutureProvider<List<Category>>((ref) async {
  final groups = await ref.watch(equnForumGroupsProvider.future);
  return EqunDiscuzAdapter.forumGroupsToCategories(groups);
});
...
final categoryTopicsProvider = FutureProvider.family<TopicListResponse, String>(
  (ref, slug) async {
    final fid = int.tryParse(slug.replaceFirst(RegExp(r'^forum-'), ''));
    if (fid == null || fid <= 0) {
      return TopicListResponse(topics: const [], moreTopicsUrl: null);
    }
    final page = await ref
        .watch(equnDiscuzServiceProvider)
        .fetchForumTopics(fid);
    return EqunDiscuzAdapter.forumTopicsToResponse(page, hasMore: false);
  },
);
