...
class _StatsCardContent extends ConsumerWidget {
  ...
  _ResolvedData _resolve(WidgetRef ref) {
    switch (config.dataSource) {
      ...
      case StatsDataSource.equn:
        final state = ref.watch(equnDiscuzProfileProvider);
        return _ResolvedData(
          values: _fromEqun(state.value),
          isLoading: state.isLoading && !state.hasValue,
  ...
  Map<ProfileStatType, int> _fromEqun(EqunDiscuzProfile? profile) {
    if (profile == null) return {};
    return {
      ProfileStatType.points: profile.points,
      ProfileStatType.basicScore: profile.basicScore,
      ProfileStatType.wikiPuzzle: profile.wikiPuzzle,
      ProfileStatType.competitionScore: profile.competitionScore,
      ProfileStatType.contribution: profile.contribution,
    };
  }
}
...
String getStatLabel(ProfileStatType stat) {
  switch (stat) {
    ...
    case ProfileStatType.points:
      return S.current.profileStats_points;
    case ProfileStatType.basicScore:
      return S.current.profileStats_basicScore;
    case ProfileStatType.wikiPuzzle:
      return S.current.profileStats_wikiPuzzle;
    case ProfileStatType.competitionScore:
      return S.current.profileStats_competitionScore;
    case ProfileStatType.contribution:
      return S.current.profileStats_contribution;
  }
}
...
String getDataSourceLabel(StatsDataSource source) {
  switch (source) {
    ...
    case StatsDataSource.equn:
      return S.current.profileStats_sourceEqun;
  }
}
