...
class _CategorySelectionSheetState extends State<CategorySelectionSheet> {
  ...
  List<_CategoryItem> _buildList() {
    ...
    final availableIds = {for (final cat in _availableCategories) cat.id};
    for (final cat in _availableCategories) {
      final parentId = availableIds.contains(cat.parentCategoryId)
          ? cat.parentCategoryId
          : null;
      childrenMap.putIfAbsent(parentId, () => []).add(cat);
    }
