...
class OrderDropdown extends StatelessWidget {
  ...
  final DropdownStyle style;
  final List<TopicSortOrder>? options;
  ...
  Widget build(BuildContext context) {
    final colorScheme = Theme.of(context).colorScheme;
    final effectiveOptions = options ?? TopicSortOrder.values;
    final effectiveOrder = effectiveOptions.contains(currentOrder)
        ? currentOrder
        : effectiveOptions.first;
    final isActive = effectiveOrder != TopicSortOrder.defaultOrder;
    ...
    return SwipeDismissiblePopupMenuButton<TopicSortOrder>(
      ...
      offset: const Offset(0, 36),
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
      tooltip: S.current.topic_sortTooltip(effectiveOrder.label),
      itemBuilder: (context) {
        return effectiveOptions.map((order) {
          final isSelected = order == effectiveOrder;
      ...
      child: style == DropdownStyle.compact
          ? _buildCompactChild(colorScheme, isActive, effectiveOrder)
          : _buildNormalChild(colorScheme, isActive, effectiveOrder),
    );
  }
