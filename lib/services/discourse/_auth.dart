...
mixin _AuthMixin on _DiscourseServiceBase {
  ...
  /// Equn Discuz 登录成功后更新兼容层登录态并广播刷新。
  Future<void> onDiscuzLoginSuccess(
    String username, {
    ...
    final submittedProfile = await saveDiscuzProfile(
      (profile ?? EqunDiscuzProfile(username: normalized)).merge(
        EqunDiscuzProfile(username: normalized),
      ),
    );
    final nextProfile = await fetchCurrentDiscuzProfile() ?? submittedProfile;
    _tToken = null;
    _credentialsLoaded = false;
    currentUserNotifier.value = EqunDiscuzSession.buildFallbackUserFromProfile(
      nextProfile,
    );
    AuthIssueNoticeService.instance.clearSessionCookieRepairHint();
    _authStateController.add(null);
  ...
  Future<void> logout({bool callApi = true, bool refreshPreload = true}) async {
    ...
    await _storage.delete(key: DiscourseService._discuzProfileKey);
    _credentialsLoaded = false;
