...
class ExportUtils {
  ...
  /// 获取主帖列表，与 [exportTopic] 共享同一批分批/限流逻辑。
  ...
  static Future<String> _exportToMarkdown(
    ...
    buffer.writeln();
    buffer.writeln('> 来源: ${EqunThreadUrl.thread(detail.id)}');
    buffer.writeln();
    buffer.writeln('---');
    buffer.writeln();
  ...
  static String _exportToHtml(TopicDetail detail, List<Post> posts) {
    ...
    // 标题
    final sourceUrl = EqunThreadUrl.thread(detail.id);
    buffer.writeln('<header>');
    buffer.writeln('<h1>${_escapeHtml(detail.title)}</h1>');
    buffer.writeln(
      '<p class="source">来源: <a href="$sourceUrl">$sourceUrl</a></p>',
    );
    buffer.writeln('</header>');
